...
//System.err.println(" "+colour);       
    ...
    /** Finds the most saturated colour for the given hue.
     *  @param hue Hue whose most saturated colour is to be found. This should be a value in degrees.
     *  @return Colour expressed in LCh coordinates.
     */ 
    private double[] getMostSaturatedColourOld(double hue)
    {
        double hueRad = hue*ColourConverter.DEG2RAD;
        double alpha = -Math.sin(hueRad);
        double beta =  Math.cos(hueRad);
        double gamma = 1;
        
        double un = 4*wp.getTristimulus()[0] / (wp.getTristimulus()[0] + 15*wp.getTristimulus()[1] + 3*wp.getTristimulus()[2]);
        double vn = 9*wp.getTristimulus()[1] / (wp.getTristimulus()[0] + 15*wp.getTristimulus()[1] + 3*wp.getTristimulus()[2]);
        
        
        double red=0,grn=0,blu=0;
        double mRX = ColourConverter.M[0][0];
        double mRY = ColourConverter.M[1][0];
        double mRZ = ColourConverter.M[2][0];
        double mGX = ColourConverter.M[0][1];
        double mGY = ColourConverter.M[1][1];
        double mGZ = ColourConverter.M[2][1];
        double mBX = ColourConverter.M[0][2];
        double mBY = ColourConverter.M[1][2];
        double mBZ = ColourConverter.M[2][2];
        
        if ((hue>=0) && (hue <60))
        {
            // Between Red and Yellow (RG) [1,n,0]
            red = 1;
            grn = Math.pow(-((alpha*un+beta*vn)*(mRX+15*mRY+3*mRZ)-(4*alpha*mRX+9*beta*mRY))/
                            ((alpha*un+beta*vn)*(mGX+15*mGY+3*mGZ)-(4*alpha*mGX+9*beta*mGY)),
                            1/gamma);
            blu = 0;
        }
        else if ((hue>=60) && (hue <120))
        {
            // Between Yellow (RG) and Green [n,1,0]
            red = Math.pow(-((alpha*un+beta*vn)*(mGX+15*mGY+3*mGZ)-(4*alpha*mGX+9*beta*mGY))/
                    ((alpha*un+beta*vn)*(mRX+15*mRY+3*mRZ)-(4*alpha*mRX+9*beta*mRY)),
                    1/gamma);
            grn = 1;
            blu = 0;
        ...
        {
            // Between Green and Cyan (GB) [0,1,n]
            red = 0;
            grn = 1;
            blu = Math.pow(-((alpha*un+beta*vn)*(mGX+15*mGY+3*mGZ)-(4*alpha*mGX+9*beta*mGY))/
                    ((alpha*un+beta*vn)*(mBX+15*mBY+3*mBZ)-(4*alpha*mBX+9*beta*mBY)),
                    1/gamma);
        }
        else if ((hue>=180) && (hue<240))
        {
            // Between Cyan (GB) and Blue [0,n,1]
            red = 0;
            grn = Math.pow(-((alpha*un+beta*vn)*(mBX+15*mBY+3*mBZ)-(4*alpha*mBX+9*beta*mBY))/
                            ((alpha*un+beta*vn)*(mGX+15*mGY+3*mGZ)-(4*alpha*mGX+9*beta*mGY)),
                            1/gamma);           
            blu = 1;
        ...
        {
            // Between Blue and Magenta (RB) [n,0,1]
            red = Math.pow(-((alpha*un+beta*vn)*(mBX+15*mBY+3*mBZ)-(4*alpha*mBX+9*beta*mBY))/
                            ((alpha*un+beta*vn)*(mRX+15*mRY+3*mRZ)-(4*alpha*mRX+9*beta*mRY)),
                            1/gamma);
            grn = 0;
            blu = 1;
            //double rgb[] = ColourConverter.RGBtoXYZ(new double[] {1,0,0});
            //System.err.println("RGB: "+rgb[0]+","+rgb[1]+","+rgb[2]);
        }
        else if ((hue>=240) && (hue<=360))
        {
            // Between Magenta (RB) and Red [1,0,n]
            red = 1;
            grn = 0;
            blu = Math.pow(-((alpha*un+beta*vn)*(mRX+15*mRY+3*mRZ)-(4*alpha*mRX+9*beta*mRY))/
                            ((alpha*un+beta*vn)*(mBX+15*mBY+3*mBZ)-(4*alpha*mBX+9*beta*mBY)),
                            1/gamma);           
        }
        else 
        {
            System.err.println("Unexpected hue: "+hue);
        }
        ...
        /*
        Color rgbMSC = Color.getHSBColor((float)hue/360, 1, 1);

        
        if ((rgbMSC.getRed() < 255) && (rgbMSC.getRed() > 0))
        {           
            red = Math.pow(-((alpha*un+beta*vn)*(M[0][2]+15*M[1][2]+3*M[2][2])-(4*alpha*M[0][2]+9*beta*M[1][2]))/
                            ((alpha*un+beta*vn)*(M[0][0]+15*M[1][0]+3*M[2][0])-(4*alpha*M[0][0]+9*beta*M[1][0])),
                            1/gamma);
    
            grn = rgbMSC.getGreen()/255.0;
            blu = rgbMSC.getBlue()/255.0;
            
        }
        else if ((rgbMSC.getGreen() < 255) && (rgbMSC.getGreen() > 0))
        {
            red = rgbMSC.getRed()/255.0;
            grn = 0.5;
            blu = rgbMSC.getBlue()/255.0;
        }
        else if ((rgbMSC.getBlue() < 255) && (rgbMSC.getBlue() > 0))
        {
            red = rgbMSC.getRed()/255.0;
            grn = rgbMSC.getGreen()/255.0;
            blu = 0.5;
        ...
        {
            // We are at one of the corners of the colour cube.
            red = rgbMSC.getRed()/255.0;
            grn = rgbMSC.getGreen()/255.0;
            blu = rgbMSC.getBlue()/255.0;
        }
    ...
    public static void main(String[] args)
    {
        ...
        double[] msc = converter.getMostSaturatedColourOld(280);
